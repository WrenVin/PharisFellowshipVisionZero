...
> **Scope: surface streets across the full-purpose City of Houston** (66,922 full-service segments served to the dashboard), rebuilt 2026-07-02 (area set by `AREA` in `src/config.py`; `seg_id` prefix `H-` from `cfg.SEG_PREFIX`). The study-area boundary is the City's **full-purpose (full-service) service area** — COH GIS "Houston City Limit" (Administrative_Boundary layer 0), the polygons tagged `SERVICE_TY='FULL'`, dissolved into one ~598 sq mi boundary (pulled by `src/fetch_boundary.py`); the **limited-purpose annexation slivers are excluded** (the City provides only limited services there and CRIS does not code their crashes to Houston, so they held essentially no data). The enriched analysis gpkg keeps the comprehensive ~75k-segment network; `export_webmap_data.py` clips the served data to this boundary. Only limited-access freeways/tollways are excluded; at-grade arterials including state-owned ones (S Main/US-90A, SH 6, Westheimer/FM 1093) are kept and **labeled** `on_txdot` (city vs TxDOT ownership, from TxDOT's roadway inventory) rather than dropped — matching Austin's Vision Zero dashboard and the City's own HIN. ~11% of the surface-street KSI shown are on TxDOT-owned arterials (the rest city streets; freeways, which are state-owned, are excluded entirely). `on_txdot` is set only where a segment runs *along* a TxDOT on-system roadway (within 60 ft AND bearing within 30 deg for >=50% of its length); interstates (`RTE_PRFX` 'IH') are dropped from the match set and a hand-curated list of ~42 false-positive `seg_id`s is forced city-owned, so streets that merely cross over or run under a freeway are not mislabeled. 974 segments are labeled TxDOT. Processed filenames are `houston_*.gpkg`. **Land use (`landuse_*`) is deferred** at city scale and absent from the current data.
...
| `houston_crashes.gpkg`, layer `crashes` | **Cleaned crash points** (TxDOT CRIS), deduped, geocoded, clipped to the City of Houston, severity-classified, and carrying the pipeline's crash→segment assignment (`seg_id`, null beyond 200 ft). Built by `src/build_crashes.py`; `seg_id` written back by `src/assign_crashes.py`. | Crash analysis; the web export's per-crash street attribution. |
| `houston_segments_model.gpkg`, layer `segments` | **The modeling layer** (gitignored; regenerates in seconds via `src/model_spatial_baseline.py`): the enriched network restricted to the 66,922 full-purpose segments (outside the boundary CRIS coverage is ~zero, so those zeros are missing data, not safety), plus `on_hin`/`on_txdot` and the step-1 hotspot columns — `gi_z`/`gi_p` (Getis-Ord Gi* z-score / pseudo-p under shared-endpoint weights, 999 permutations), `hotspot` (significantly high after BH-FDR α=0.05), `hotspot_band` (same under the 1,000-ft distance-band weights, sensitivity). | All modeling (steps 1–4). |
...
| `seg_id` | text | `H-…` / null | **The pipeline's crash→segment assignment** (written back by `src/assign_crashes.py`): the single nearest segment within 200 ft, with merged-away halves credited to their representative. Null when no street is within 200 ft (~2% of crashes). The web export reuses this — it does not re-derive its own — so the dashboard's per-crash counting reconciles exactly with the per-segment `n_*` columns. |
