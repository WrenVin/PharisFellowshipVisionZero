...
- **Reviewer's structure adopted: primary common holdout 2023 to June 2026** (postdates every map, including the HIN's 2018-2022 selection window) plus secondary 2022+ deployment window. Primary result: **v2 51% vs HIN 46%** at 589 mi (the HIN loses 3 points once its free overlap year is removed); v1 48; null 43; Gi* 39 (from 54 in-sample). SN block-bootstrap CI for v2 minus HIN: [-0, +10] — still touches zero, so still claimed as a match per the pre-registered rule. Secondary window unchanged (51 vs 49, CI [-4, +7] under block bootstrap).
- **Imagery freeze integrity (the reviewer's "potentially fatal" item): defended with data.** The corpus is overwhelmingly mid-2010s (median capture year 2015); only 370 of 18,133 imagery segments (2.0%) have a median capture year after 2021. Masking imagery features on those segments (honest-fill) leaves capture unchanged; a stricter 2019 mask moves it one point. Per-image manifest remains on the PC; segment-median masking is the conservative Mac-side bound.
- **Imagery ablation: the coverage-geography attack is dead.** v1 + availability flags only (sv_missing, pano share) captures exactly v1's 47%; the extracted visual features carry the full gain to 49; within the imagery subset (availability constant) v2 beats v1 61 to 64. The gain is visual content.
- **Forward test formalized, and the matched comparator killed the strong version of the claim.** Set-by-period NB interaction (SN-clustered): vs all other streets IRR 1.19 [1.06, 1.34] (2022+) and 1.32 [1.16, 1.49] (2023+) — the citywide-trend claim is now formal. Vs matched off-HIN arterials/collectors: IRR 1.00-1.02, null. The overlooked streets worsened with their functional class, not beyond it. Paper reframed: the model's contribution is locational (which streets concentrate the deteriorating class's harm, found from design alone), not trajectory-level foresight. Disclosed in 5.4, Limitations, and DEFENSE Layer 9.
- **Absorption upgraded:** against the matched base (other off-HIN arterials/collectors, 6.6% [5.2, 8.3]), the overlooked set's 20.8% [17.1, 24.9] absorption into the 2025 HIN is a **3.2x lift [2.5, 3.9]** — stronger than the old 2.1x and now carrying a CI.
- **Statutes verified online:** the camera-enforcement claim is rescued with two citations — Transp. Code Sec. 542.2035 (2007, HB 922) bans municipal speed cameras; HB 1631 (2019) ended red-light cameras. The draft's single-statute attribution was wrong and is fixed.
- **Paper sweep completed:** primary/secondary temporal structure; abstract rewritten (~296 words); Table 1 reframed as conditional associations for prediction (causal wording removed per reviewer); lanes/SD unit fix; LODO pooling made explicit; GBM labeled untuned reference; monotone decile calibration stated (and plotted: `fig_calibration.png`, in-paper placement pending page budget); softened HIN-only tier language; CRIS extraction date noted. `fig_temporal_capture.png` regenerated on the primary window.
- Not done yet: docx rebuild (stale; final conversion happens against the TRB template anyway), deck sync (the lightning-talk deck reports the deployment window and remains internally consistent), EB-hybrid future-work paragraph.
...
**1. Primary-source verification of the comparator.** A full read of the VZAP (Nov 2020) and the 2022 Annual Report settled the HIN's provenance: built on **2014-2018 crash data** ("Crash data in this plan are from 2014-2018 unless otherwise noted," VZAP p. 07; the City's own map layers say "High Injury Network 2018"). The City publishes **no methodology, no formula, and no mileage** — only the "nearly 60% on 6% of streets" density framing; the 589-mile figure is this project's measurement of their GIS layer. Bonus find: **VZAP Action 2.3 commits the City to systemic risk-factor analysis** (status "Underway" in the 2022 report) — the Concept PSN is an open implementation of the City's own committed action, now cited in the PSN report, DEFENSE.md, and the paper skeleton. Vintage precision propagated everywhere (the "conservative comparison" argument is now stated as PARTIAL overlap: only 2016-2018 of the grading window overlaps the HIN's selection years).

**2. Tier 1 language fixes, all four report templates regenerated.** model_nb now reports the LR test of alpha (statistic 3,193, boundary-corrected p≈0), BIC (NB beats ZINB outright), and the zero-share calibration that closes the ZINB question permanently (observed 90.8% zeros vs NB-predicted 90.9%); the IRR table is partitioned into design features vs adjustment covariates (Westreich & Greenland); the underreporting-absorption defense and the aggregate over-prediction (+10%, rank-based reading rule) are stated. The v2 "DIRECT design effects" claim is retracted in docstring and report.

**3. Tier 2 sensitivity batch (`src/audit_sensitivities.py` → report).** Passed: 2018+ severity refit (drift 12.2% on an insignificant dummy; overlooked-set Jaccard 0.85); buffer sensitivity (9,565/9,711/9,756 severe at 100/200/250 ft, r≥0.994); imagery-vintage control (vehicle IRR unchanged at 1.24); **fold-level v2 consistency (v2 beats v1 in 10/11 folds, sign test p=0.012, mean +2.6 points — the v2 gain is real)**. Mixed, reported honestly: Gi* per-mile (95.3% classification agreement but top-set Jaccard 0.56 — crash-map membership is method-sensitive, echoing the 43% Gi*-HIN overlap); ADT reweighting (drift 108%→83%: composition explains only part).

**The finding that overturned: the design-only equity overlay SUSTAINED the audit's circularity attack.** With demographic covariates removed, the overlooked set's income skew disappears and mildly reverses (design-only median income $80.3k vs citywide $69.6k; the full model's $61.4k skew is substantially produced by its own income/poverty coefficients steering predicted risk toward poor areas). All project text revised the same day: the equity profile is now stated as a **description of the deployed score with the mechanism disclosed**, never a design or process finding. The danger finding is unaffected (the overlooked set's temporal-holdout worsening of 17% vs trend does not depend on why its income profile arises). This is the audit system working: the check was pre-committed, it failed the convenient reading, and the honest version shipped within hours.

Also: the HIN-vs-PSN side-by-side is productized (`src/plot_hin_vs_psn.py` → reports/hin_vs_psn.png), the deck is regenerated with corrected language, DEFENSE.md statuses are updated throughout (fix queue: Tier 1 complete; Tier 2 complete except the off-street image filter, which needs the per-image table on the PC, and the MI-ADT sensitivity; Tier 3: temporal done, land use / DAG redraw / 2048px / tuned benchmark remain).
...
**Vetted data refresh.** Re-ran `export_webmap_data.py` + `export_vz_summary.py`. The published set moves 421,570 → **421,679 crashes** and 66,917 → **66,922 segments** (~6,421 mi): the full-purpose boundary was refined on 2026-06-22 *after* the last export, so the committed files were clipped with the old boundary. Published KSI now reconciles **exactly** with the analysis build (9,928 = 9,928; fatal 1,687 = 1,687), and the old 48-orphan-seg_id warning is gone. `vz_summary.generated` → 2026-07-02; the dashboard's "Compiled …" footer now derives from that field (was a hardcoded "June 2026"), so it can't go stale again.

**A real consistency bug, found and fixed.** The dashboard's `computeCounts()` has two paths: the default view colors streets from the segment file's pre-baked `n_severe` (pipeline assignment, **200-ft cap** → sums to 9,720), but any temporal narrowing switches to counting `crash_points.json` seg_ids — which the export assigned with an **unbounded** nearest-join (→ 9,928). Touching a slider therefore subtly shifted street counts (the ~2% of KSI crashes >200 ft from any street). Fix at the source of truth: `assign_crashes.py` now **persists its per-crash assignment** (`seg_id`, null beyond 200 ft, merged-away halves credited to reps) back onto `houston_crashes.gpkg`, and `export_webmap_data.py` **reuses that assignment** instead of re-deriving its own (nulling ids outside the published subset; legacy fallback kept). Verified in the live page: both paths now sum to 9,720 with **zero per-segment disagreements** across all 66,922 segments. Crash `on_hin`/`on_txdot` follow the same assignment (null → False); KSI on TxDOT arterials 1,109 → **1,090** (~11%, unchanged rounded).

**How the dashboard was verified** (the methodology, for the record):
1. **CI contract** (`tests/validate_exports.py`): parse/shape/bbox/flags, internal consistency of `vz_summary`, cross-file reconciliation, equity guard — green, 0 warnings.
2. **Independent recomputation** (`tests/verify_dashboard_math.py`, new): a from-scratch Python re-implementation of every displayed computation — toll, YLL, FHWA econ/comprehensive cost, Gini + Lorenz readouts, mode/income/ownership splits, worst-streets ranking, per-year series — run against the published files.
3. **Live-page comparison**: loaded the dashboard in a browser and read the *rendered* values (KPI DOM text and the internal `tally()`/`concentration()`/`costTotal()` outputs), then diffed against (2). Citywide: all exact (killed 1,687, KSI 9,928, crashes 421,679, YLL 69,513, $15.3B/$64.8B, ped 597/2,226, bike 89/459, Gini 0.94, half-of-harm 2.3%). Filtered states: `?mode=ped` (597/2,226/8,772/YLL 24,509) and `?district=C` (88/712/41,177/YLL 3,592) — all exact. Worst-5 (Westheimer 264, Bissonnet 148, Richmond 140, Main 126, Tidwell 107) reproduced exactly (name-grouped, `n_severe` basis). Two-path `computeCounts()` equality checked per segment. Console: zero errors/warnings.
4. **Distance audit**: confirmed the 9,928 vs 9,720 gap is the 200-ft rule (217 KSI crashes >200 ft from a published street), not data loss.

**Tooltip accuracy fix.** The concentration KPI tooltip claimed "at the 6% threshold the City uses for its HIN, 82% of KSI fall on those streets" — but 82% is the *density-ranked best-possible* 6% of this view, not the City's actual HIN (~71% per the count-ranked reconstruction in `vz_summary`). Reworded to "the deadliest 6% of street-miles in this view — the same share of the network the City sized its High Injury Network to — carry X%."

**Code sweep** (subagent-audited): removed unused imports (`geopandas` in `pull_osm.py`, `json` in `fetch_boundary.py`); the June cleanup had caught everything else. Kept `config.py`'s side-by-side area example (intentional). Docs: README (status date 2026-07-02, published 66,922/~6,421 mi, analysis-vs-published crash counts, TxDOT KSI 1,090, single-dashboard wording, corrected payload sizes ~31+~36 MB, tests map entry), CODEBOOK (66,922, rebuilt 2026-07-02, TxDOT segments 974 — was a stale ~1,438 — new `seg_id` crash column documented, deep-check note), ELI5 unchanged (its numbers now match the live page exactly).

**Known, deliberate:** the dashboard's toll KPIs count **crashes** (fatal / serious-injury crashes), matching `vz_summary` and all docs; person-level counts (people killed 1,780, people seriously injured 9,596, per CRIS injury counts) are higher because a crash can hurt more than one person. Whether to relabel or switch the KPI basis is an open product decision — flagged, not changed.
...
- **Follow-up (y-axis fit).** All-crashes counts are 10–40× larger than KSI, so the y-axis tick labels (e.g. 41,500) overran the chart's narrow left margin and clipped. Added `fmtTick()` to render ticks compactly (`44k`, `8.4k`, `1.1k`) and applied it to every bar chart's gridline labels. By-year now reads 0 / 22k / 44k, time-of-day 0 / 17k / 33k, day-of-week 0 / 34k / 69k; KSI mode reads 0 / 550 / 1.1k.
- **Follow-up (remaining panels).** The **travel mode**, **neighborhood income**, and **road owner** panels also hard-filtered to KSI (`p[2]!==1 continue`, or `tally`'s KSI-only fields), so they went blank for a no-KSI selection in all-crashes mode. Gated each on `sev` (`if(sev==='ksi' && p[2]!==1) continue`), added `ped_all`/`bike_all` to `tally()` so the mode split has crash-level pedestrian/bike counts, and made every sub-line / legend / note switch noun ("All crashes" / "of crashes" / "No crashes …"). The **concentration** (Lorenz/Gini) and **most-affected-streets** panels were already mode-aware (both read `activeCounts`, which `computeCounts` builds from `sev`). Verified citywide: mode 409,506 veh / 8,771 walk / 3,293 bike, income 77% under-$100k, owner 385,668 city / 35,902 TxDOT (all sum to 421,570); East 11th Street (0 KSI, 37 crashes) now populates all panels in all-crashes mode and correctly reads "No KSI …" in KSI mode. No console errors.
- **Follow-up (concentration KPI).** Confirmed the concentration statistic is a genuine Gini coefficient — the standard Lorenz-curve `2·area − 1` (same calc as `src/analyze_concentration.py`); the Gini is a general inequality measure, not income-specific, so the term stays. The KPI card gained a second line (and jumped) when the metric word changed length (KSI → crashes) and `· Gini 0.83` overflowed the ~159px sub. Fixed by shortening the KPI sub to the mode-independent **"of streets, half the harm"** (136px, one line, byte-identical in both modes so it can never jump) and dropping the Gini value from the KPI — it stays in the Lorenz panel below, which already shows "Half of all crashes is on 3.8% of street-miles · Gini 0.83".
