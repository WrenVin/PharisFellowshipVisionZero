...
# Freeway-facility filter: crashes on freeways/tollways were being snapped onto
# nearby city cross-streets. Drop them — they're TxDOT facilities, not the city
# streets this project studies. Road_Cls_ID 1=Interstate 6=Tollway 8=TollBridge
# 9=NonTrafficway; Rpt_Road_Part_ID 2=service/frontage 3/4=ramp 5=connector
# (freeway-only parts); class 2 (US/State hwy) is mixed, so drop it only when the
# street name is a freeway (keeps surface arterials like Shepherd, Kirby, Braeswood).
...
# drop freeway-facility crashes (see FWY_NAME note above)
...
- In District C (geocoded): {n_in_district:,}
- **Excluded {n_fwy:,} freeway/tollway-facility crashes** (TxDOT, not city streets) — they were being snapped onto cross-streets
- **City-street crashes in District C: {len(g):,}** (EPSG:2278)
