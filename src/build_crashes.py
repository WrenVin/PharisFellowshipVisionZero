...
# Roadway-ownership filter: this project studies streets the CITY owns and can
# redesign. CRIS classifies every crash's roadway in Road_Cls_ID:
#   1 Interstate · 2 US & State Hwy · 3 Farm-to-Market · 4 County Road
#   5 City Street · 6 Tollway · 7 Other · 8 Non-trafficway · 9 Other
# We keep ONLY class 5 (City Street). This authoritatively drops TxDOT roads
# (Interstates, US/State highways like S Main/US-90A and SH 6, FM roads like the
# western Westheimer/FM 1093, tollways) and county roads, matching the City's
# own High Injury Network scope. Replaces the earlier freeway-name heuristic,
# which still let at-grade US/State/FM highways through.
CITY_STREET_CLS = 5
...
# keep only city-owned streets (CRIS Road_Cls_ID == 5); see note above
cls = pd.to_numeric(g["Road_Cls_ID"], errors="coerce")
is_city = cls.eq(CITY_STREET_CLS)
n_noncity = int((~is_city).sum())
noncity_by_cls = cls[~is_city].value_counts().sort_index().to_dict()
g = g[is_city].copy()
...
- **Excluded {n_noncity:,} non-city-owned crashes** (CRIS Road_Cls_ID != 5: Interstate/US-State/FM/County/Tollway = TxDOT or county, not city streets). By class: {noncity_by_cls}
- **City-street crashes (Road_Cls_ID = 5): {len(g):,}** (EPSG:2278)
