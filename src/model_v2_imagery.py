...
Estimand note (per the DAG, corrected after the 2026-07-03 audit): v2 was
intended to move coefficients toward direct design effects by measuring
pedestrian exposure. The available proxy proved too weak (person counts null;
sparse, low-resolution mid-2010s imagery), so v2 coefficients remain, like
v1's, TOTAL design effects with the pedestrian-exposure artifact only
partially attenuated. A direct-effect reading would additionally require no
unmeasured confounding of the pedestrian-exposure-to-crash path, which the
latent land-use node precludes. The sidewalk coefficients falling as exposure
is partially measured is claimed only as directional evidence for the
artifact hypothesis.
...
plus `sv_missing` flag elsewhere). Estimand: v2 coefficients remain TOTAL
design effects. The pedestrian-exposure instrument came back null (sparse,
low-resolution mid-2010s imagery cannot count pedestrians reliably), so the
exposure adjustment v2 was designed for did not materialize; the sidewalk
artifact is only partially attenuated and the standing caveat applies
unchanged. The null is an instrument limitation, not a finding about walking.

## 1. The sidewalk test

| Variable | v1 (exposure unmeasured) | v2 (exposure partially measured) |
|---|---|---|
{sw_lines}
...
The sidewalk coefficients move in the predicted direction and no further:
directional evidence for the exposure-artifact hypothesis, not resolution.
